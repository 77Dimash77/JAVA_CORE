...
public class z2 {
    ...
    private static Order[] orders = new Order[10]; // Увеличили размер массива
    ...
    public static void main(String[] args) {
        ...
        while (orderCount < orders.length && i < info.length) {
            try {
                orders[orderCount] = buy((Buyer) info[i][0], (Product) info[i][1], (int) info[i][2]);
                orderCount++;
            } catch (ProductException e) {
                e.printStackTrace();
            } catch (OrderException e) {
                orders[orderCount++] = buy((Buyer) info[i][0], (Product) info[i][1], 1);
            } catch (BuyerException e) {
                throw new RuntimeException();
            } finally {
                System.out.println("Заказ сделан емкостью на: " + orderCount);
            }
            ++i;
        }
    }
